    ...
    <div class="row">
        ...
        <div class="col-md-11 col-lg-11">
            <div class="text-center mb-5">
                @if(isset($persona))
                    <h3>Actualizar Proveedor {{ $persona->nombre}}</h3>
                @else
                    <h3>Agregar Proveedor</h3>
                @endif
            </div>
            <form action="{{ isset($persona)? route('proveedor.update', $persona->idpersona):route('proveedor.store') }}" method="post">
                @if(isset($persona))
                    @csrf
                    @method('PUT')
                @else
                ...
                <a href="{{ route('proveedor.index') }}" value="Cerrar" class="btn btn-danger">Cerrar</a>
            </form>
        </div>
    </div>
</div>
